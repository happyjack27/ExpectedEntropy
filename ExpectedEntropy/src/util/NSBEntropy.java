...
*/
	...
	public double _xi(double beta, double K) {
		return psi(0,K*beta+1)-psi(0,beta+1);
	}
	public double _dxi(double beta, double K) {
		return K*psi(1,K*beta+1)-psi(1,beta+1);
	}
	public double _dxi(double beta, double K) {
		return K*psi(1,K*beta+1)-psi(1,beta+1);
	}
	public double _S1i(double x, double[] nxkx, double N,double kappa) {//, double K) {
		return nxkx[x] * (x+beta)/(N+kappa) * (psi(0,x+beta+1)-psi(0,N+kappa+1));//K*psi(1,K*beta+1)-psi(1,beta+1);
	}
	public double _S1(double beta, double[] nxkx, double N,double K) {//, double K) {
		double kappa = beta*K;
		
		double d = 0;
		for( int i = 0; i < nxkx.length; i++) {
			d += _S1i(nxkx[i],nxkx,beta,N,kappa);
		}
		return d;
	}
	public double _rhoi() {
